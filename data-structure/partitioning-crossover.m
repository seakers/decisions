for i=1:nKids
    ...
    existing_sat     = mama(ceil(GenomeLength/2)+1:end) <= max(mama(1:ceil(GenomeLength/2))); %existing_sat(i) = 1 means that instrument i is in a satellite that was created in the first half of mama

    kid_sat          = [first_half_papa;second_half_mama];         % Try to combine satellites of second half with satellite of first half
    ...
    for j = 1:nsat2                                                % foreach satellite from mum
        ...
        for k = 1:length(sat)                                      % loop through its instruments
            instr = sat(k);                                        % instr
            if existing_sat(instr-ceil(GenomeLength/2))            % if this instrument in mum belongs to a satellite of the 1st half of mum, lump this satellite with a random satellite from first half (which comes from dad)
    ...
    if constr
        feasible = isTrialFeasible(xoverKids(i,:),linCon.Aineq,linCon.bineq,linCon.Aeq, linCon.beq,linCon.lb,linCon.ub,sqrt(options.TolCon));
        if ~feasible % Kid is not feasible
            % Children are arithmetic mean of two parents (feasible w.r.t linear constraints)
            alpha = rand;
